# Anthony Henry
> Looking for SDE roles <br>



## About
- BSIT/BSCS @ Kennesaw State (Enterprise Systems & Data - HPC & Parallel Programming)

- MSCS @ GA Tech (Computing Systems & Human-Computer Interaction)




## MY STACK

<p align="center">
  <a href="https://skillicons.dev">
    <img src="https://skillicons.dev/icons?i=go,terraform,kubernetes,docker,vscode,vscodium,postgres,azure,aws,firebase,supabase,heroku,bash,cypress,linux,redhat,git,github,gitlab,npm,js,ts,vite,vitest,mongodb,express,react,nodejs,prisma,py,selenium,postman,fastapi,flask,java&perline=5" />
  </a>
</p>



<br>

```lua
Frontend: Vite, React 19, TypeScript 5.9

API Gateway: Go 1.21+, Gin framework, structured logging

Microservices:
  - Go (data processing, external API calls)
  - Node.js (if specific npm library needed)
  - Python (where scraping is required)

Database: PostgreSQL 16, Azure Database

Infrastructure: Azure Container Apps, GitHub Actions, Terraform
```
